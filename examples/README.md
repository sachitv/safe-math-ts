...
- `quat_from_rotation_matrix.test.ts`: recover a quaternion from a rotation
  matrix, read named components via `quatX/Y/Z/W`, and verify the safe variant
  rejects non-orthonormal matrices.
- `mat4_pose_decomposition.test.ts`: extract the translation column and
  orientation quaternion from a rigid-transform matrix via `mat4Translation()`
  and `mat4Quat()`, with a round-trip check and a rejection test for scale
  matrices.
